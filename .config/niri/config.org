...
#+property: header-args:kdl :tangle "~/.config/niri/config.kdl"

** Environment Variables

yes, i got nvidia gpu and i use wayland

#+begin_src kdl :tangle yes
environment {
  DISPLAY ":0"
...
i don't like the default layout, this one feel much better to me
...
yep, i don't use a lot of keybinds, just the absolute necessities
...
the input system the way i like
...
just the right amount of blur/transparency/floating
...
window-rule {
  ...
  match app-id=r#"^foot|footclient|St|st$"#
...
yeah, sometimes when i connect to the external monitor, i disable the laptop's monitor
...
these feel smooth without being too glittery like some of the hyprland configs out there, if you know what i mean
...
yeah, i use the old way to startup my programs and settings than using systemd services

#+begin_src kdl :tangle yes
spawn-at-startup "~/.config/autostart/startup.sh"
spawn-at-startup "xrdb -merge ~/.config/X11/xresources"
...
spawn-at-startup "gsettings set org.gnome.desktop.interface cursor-theme 'Bibata-Modern-Ice'"

spawn-at-startup "gsettings set org.gnome.desktop.interface gtk-enable-primary-paste false"
#+end_src
...
#+begin_src kdl :tangle yes
screenshot-path "~/media/pix/ss/shot_%y%m%d_%H-%M-%S.png"
